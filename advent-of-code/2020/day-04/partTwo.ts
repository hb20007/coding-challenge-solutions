...
const isPassportValid = (passport: Array<string>): boolean => {
  ...
  const results = {
    isByrValid: byr &&
      isInRange(Number.parseInt(byr), Limits.Byr.MIN, Limits.Byr.MAX),
    isIyrValid: iyr &&
      isInRange(Number.parseInt(iyr), Limits.Iyr.MIN, Limits.Iyr.MAX),
    isEyrValid: eyr &&
      isInRange(Number.parseInt(eyr), Limits.Eyr.MIN, Limits.Eyr.MAX),
