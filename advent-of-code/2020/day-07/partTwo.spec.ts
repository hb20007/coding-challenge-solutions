...
Deno.test(
  `The first example rules with bag color ${
    INPUTS[0].bagColor
  } should yield child bag count ${RESULTS[0]}`,
  () => {
    assertStrictEquals(
...
Deno.test(
  `The second example rules with bag color ${
    INPUTS[1].bagColor
  } should yield child bag count ${RESULTS[1]}`,
  () => {
    assertStrictEquals(
