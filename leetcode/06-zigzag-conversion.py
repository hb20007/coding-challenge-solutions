...
class Solution:
    def convert(self, s, num_rows):
        ...
        for i in range(num_rows):
            for j in range(i, len(s), period_size): # j is the index of the first element in a particular cycle in row i. The third parameter is the update statement of the loop. We move on to evaluate the next cycle for the current row.
                res += s[j] # The first character in each row is always at the index of the row number.
                second_j = (j - i) + period_size - i # In every period, there is a second j because every row has two elements. The index of the second character in the current row in the current period is the start (i.e., first character) of the current period (j - current row, i.e., j - i) + period_size - i. This is because (i - j) + period_size is the start of the next period. If you subtract i from that, you get the character at the row we want.
                if i != 0 and i != num_rows - 1 and second_j < len(s): # There is no second j in the first and last rows, and possibly in the last period, where the string might terminate earlier than a second j would have appeared.
                    res += s[second_j]
        return res
        ...
        going_down = True
        row_idx = 0
        zig_zag_rows_array = ["" for _ in range(num_rows)] # This is bad Python practice because it creates an array of a fixed size. However, we are writing code for a very specific problem, so it's justifiable for the sake of simplicity.
        ans = ""
